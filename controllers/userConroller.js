...
const addStaff = async (req, res) => {
    ...
    if(!req.file){
      ...
      try {
        const staff = await Staff.addStaff({email, password, name, courses:course,imagename:filename});
...
const addStudent = async (req, res) => {
 ...
 const c=[courses]
  ...
  const postData={
    matricno,name,courses:c,imagename:filename
  } 
...
res.status(200).json(`${exists.name} was successfully updated with ${courses}`)
...
if (req.body){
  ...
  try {
    ...
    if (exists){
      const student= await Student.findOneAndUpdate({_id:id},{$set:{courses:exists.courses}})
    }
...
console.log(req.body)
    try {
      ...
      const staff = await Staff.findOneAndUpdate({ _id: id },{$set:{courses,email}});
